...
- [ ] **Ручная сквозная приёмка Фазы 1 — НЕ завершена.** Пользователь тестировал несколько раундов и нашёл реальные баги (CSP, ResizeObserver-цикл, диагональное дерево, фокус-зависимые стрелки, "Time"-ось графика и т.д.) — все исправлены на ветке `fix-phase-1-acceptance-findings`, самостоятельно перепроверены через CDP (реальные скриншоты + `Runtime.evaluate` живого окна: данные действительно текут, ось графика верна, дерево/доска рендерятся правильно). Но это моя собственная автоматизированная проверка, не финальное «пользователь подтвердил» — легенда графика (наведение курсора мышью на график) CDP-скриптом не проверена по-настоящему (синтетический `mousemove` не эмулирует то, чем реально пользуется uPlot для courser-tracking), и в целом финальное слово за пользователем. Не считать приёмку пройденной, пока пользователь сам не подтвердит.
