...
Следующий шаг — Фаза 2 (LLM-объяснения поверх KataGo, без RAG) — **не начинать** до явного запроса пользователя.
...
- Фаза 2 и далее — не начинать без явного запроса пользователя (см. «Постоянные правила»).
